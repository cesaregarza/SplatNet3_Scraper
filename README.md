...
[![Tests Status](./reports/junit/tests-badge.svg?dummy=8484744)](https://htmlpreview.github.io/?https://github.com/cesaregarza/SplatNet_Query/blob/main/reports/junit/report.html) ![Coverage Status](./reports/coverage/coverage-badge.svg?dummy=8484744) ![Flake8 Status](./reports/flake8/flake8-badge.svg?dummy=8484744) [![Code style: black](https://img.shields.io/badge/code%20style-black-000000.svg)](https://github.com/psf/black)
...
**SplatNet_Query** is currently under active development but is currently available on PyPI. It can be installed using pip:

```bash
pip install splatnet3_scraper
```

Note that the current versions of **SplatNet_Query** are currently `v0.x.y`, which means that the API is not guaranteed to be stable and may change in the future with the release of `v1.0.0`.

## Usage

There are two ways to use **SplatNet_Query**. The first is to use the `scraper` module, which provides a high level API that greatly simplifies the process of retrieving data from SplatNet 3. The second is to use the `base` module, which provides a low level API that allows for much more fine-grained control over the scraping process. Either way, both modules require a session token to be provided.

### Using the `scraper` module

The `scraper` module is a batteries-included module that allows queries to be made to the SplatNet 3 API with minimal effort. It is designed to be used by the end user, and as such it is the easiest and recommended way to get started with **SplatNet_Query**. The `scraper` module provides the `SplatNet_Query` class, which is used to make queries to the SplatNet 3 API. The `SplatNet_Query` class can be instantiated in one of a few ways: by providing a session token, by providing the path to a configuration file, or by loading environment variables.

#### Instantiating the `SplatNet_Query` class by providing a session token

```python
from splatnet3_scraper import SplatNet_Query
scraper = SplatNet_Query.from_session_token("session_token")
scraper.query("StageScheduleQuery")
```

#### Instantiating the `SplatNet_Query` class by providing the path to a configuration file

```python
from splatnet3_scraper import SplatNet_Query
scraper = SplatNet_Query.from_config_file(".splatnet3_scraper")
scraper.query("StageScheduleQuery")
```

#### Instantiating the `SplatNet_Query` class by loading environment variables
...
SplatNet_Query is licensed under the GPLv3. See the LICENSE file for more details.
