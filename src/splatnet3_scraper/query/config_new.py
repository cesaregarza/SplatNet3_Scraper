...
class Config:
    """The Config class is a higher-level wrapper around the ``TokenManager``
    class. It contains IO methods for saving and loading the config file to
    generate ``TokenManager`` objects. It also contains methods for getting
    options from the config file. The ``Config`` class is designed to be
    subclassed to add additional functionality, and as such it is not meant to
    function as a fully-featured config parser.
    """
